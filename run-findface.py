...
def crop_face(image_to_classify):
...
def run_inference(image_to_classify, facenet_graph):
    #
    croped_image_to_classify = crop_face(image_to_classify)
    #cv2.imshow("preprocessed", croped_image_to_classify)

    # get a resized version of the image that is the dimensions
    # SSD Mobile net expects
    resized_image = preprocess_image(croped_image_to_classify)
...
def run_images(valid_output, validated_image_filename, graph, input_image_filename_list):
    ...
    for input_image_file in input_image_filename_list :
        #print("Reading image " + input_image_file)
        # read one of the images to run an inference on from the disk
        infer_image = cv2.imread(IMAGES_DIR+input_image_file,1)
        #print(type(infer_image))
